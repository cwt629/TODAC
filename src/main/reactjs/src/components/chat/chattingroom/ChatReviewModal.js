...
const ChatReviewModal = ({ star, maxStar, counselorname, handleStarClick, handleReviewPass, handleReviewGrant, handleReviewClose }) => {
    return (
        <div>
            <span className='col_red'>{counselorname ? counselorname : '이'}</span> 상담사에게 별점을 남겨주세요 :{')'}
            ...
            <div className='review-buttons'>
                ...
                <div className='review-button bg_gray review-close' onClick={() => handleReviewClose()}>닫기</div>
            </div>
        </div >
    );
};
